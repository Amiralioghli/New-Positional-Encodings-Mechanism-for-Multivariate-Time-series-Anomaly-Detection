...
class LSTMTimeSeries(nn.Module):
    ...
    def forward(self, x):
        # Initialize hidden state and cell state for the first input
...
# Hyperparameters
input_size = 36        # Number of features in the time-series data
...
model = LSTMTimeSeries(input_size=input_size, hidden_size=hidden_size, num_layers=num_layers, num_classes=num_classes).to(device)
...
plt.title('LSTM training losses vs. validation losses')
plt.xlabel('Epochs')
plt.ylabel('Losses (%)')
...
plt.title('LSTM training and validation accuracy vs. loss', fontdict={'size': 14, 'family': 'Times New Roman'})
...
plt.title("LSTM Confusion Matrix", fontdict={'size': 14, 'family': 'Times New Roman'})
...
plt.title("LSTM AUC ROC Curve", fontdict={'size': 14, 'family': 'Times New Roman'})
