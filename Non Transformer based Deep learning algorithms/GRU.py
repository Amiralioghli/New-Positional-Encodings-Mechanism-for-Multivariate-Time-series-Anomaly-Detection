...
model = GRUTimeSeries(input_size=input_size, hidden_size=hidden_size, num_layers=num_layers, num_classes=num_classes).to(device)
...
plt.title('GRU training losses vs. validation losses')
plt.xlabel('Epochs')
plt.ylabel('Losses (%)')
...
plt.title('GRU training and validation accuracy vs. loss', fontdict={'size': 14, 'family': 'Times New Roman'})
...
plt.title("GRU Confusion Matrix", fontdict={'size': 14, 'family': 'Times New Roman'})
...
plt.title("GRU AUC ROC Curve", fontdict={'size': 14, 'family': 'Times New Roman'})
