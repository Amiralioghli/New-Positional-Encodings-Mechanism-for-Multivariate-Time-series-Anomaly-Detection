...
sequence_len=32 # sequence length of time series
...
class CNN1D(nn.Module):
    def __init__(self, input_channels, num_classes=2, kernel_size=None, padding = None, drop_prob=None, out_channels1 = None, out_channels2 = None, out_channels3 = None):
        super(CNN1D, self).__init__()
        self.drop_prob = drop_prob

        # Convolutional layers
        self.conv1 = nn.Conv1d(in_channels=input_channels, out_channels=out_channels1, kernel_size=kernel_size, padding=padding)
        self.conv2 = nn.Conv1d(in_channels=out_channels1, out_channels=out_channels2, kernel_size=kernel_size, padding=padding)
        self.conv3 = nn.Conv1d(in_channels=out_channels2, out_channels=out_channels3, kernel_size=kernel_size, padding=padding)

        # Pooling layer
        self.pool = nn.MaxPool1d(kernel_size=kernel_size)

        # Fully connected layers
        self.fc1 = nn.Linear(out_channels3 * (sequence_length // 8), 128)  # Adjust size based on sequence length
...
# Hyperparameters
input_channels = 36  # For univariate time series, this is usually 1
sequence_length = 32  # Length of the time-series sequence
learning_rate = 0.01

# Create the model, loss function, and optimizer
model = CNN1D(input_channels=input_channels, num_classes=2).to(device)
...
plt.title('Absolute PE training losses vs. validation losses')
plt.xlabel('Epochs')
plt.ylabel('Losses (%)')
...
plt.title('Absolute PE training vs. validation accuracy')
plt.xlabel('Epochs')
...
ig, ax1 = plt.subplots(figsize=(6, 3))
...
plt.title('Absolute PE training and validation accuracy vs. loss', fontdict={'size': 14, 'family': 'Times New Roman'})
...
model.eval()
#model.load_state_dict(torch.load('//Users//macbookpro//Documents//TSAD//Rotary_PE//best_model.pth')) 
test_correct = 0
...
plt.title("Absolute Positional Encoding Confusion Matrix", fontdict={'size': 14, 'family': 'Times New Roman'})
...
print("Kppa: ",cohen_kappa_score(y_test, y_pred))
...
plt.title("Absolute Positional Encodings AUC ROC Curve", fontdict={'size': 14, 'family': 'Times New Roman'})
