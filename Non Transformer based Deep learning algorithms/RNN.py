...
model = RNNTimeSeries(input_size=input_size, hidden_size=hidden_size, num_layers=num_layers, num_classes=num_classes).to(device)
...
plt.title('RNN training losses vs. validation losses')
plt.xlabel('Epochs')
plt.ylabel('Losses (%)')
...
plt.title('RNN training and validation accuracy vs. loss', fontdict={'size': 14, 'family': 'Times New Roman'})
...
plt.title("RNN Confusion Matrix", fontdict={'size': 14, 'family': 'Times New Roman'})
...
plt.title("RNN AUC ROC Curve", fontdict={'size': 14, 'family': 'Times New Roman'})
