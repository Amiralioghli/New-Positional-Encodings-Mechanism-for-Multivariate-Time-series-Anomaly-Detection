...
from Feed_Forward_and_Positional_Encoding import RelativePositionalEncoder
...
#     arXiv preprint arXiv:1803.02155. Representative attention
# ==============
...
class Shaw_MultiHeadAttention(nn.Module):
    def __init__(self, d_model, num_heads, positional_encoding="rel", dropout_rate=0.1):
        super(Shaw_MultiHeadAttention, self).__init__()
        ...
        self.query_proj = nn.Linear(d_model, self.head_dim * num_heads)
        self.key_proj = nn.Linear(d_model, self.head_dim * num_heads)
        self.value_proj = nn.Linear(d_model, self.head_dim * num_heads)
        self.out_proj = nn.Linear(d_model, self.head_dim * num_heads)

        if positional_encoding == "rel":
    ...
    def forward(self, query, key, value, mask = None, relative_bias=None, is_dropout=True):
        ...
        if self.positional_encoding == "rel":
